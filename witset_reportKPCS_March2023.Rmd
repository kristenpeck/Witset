...
One last additional check is that of serial recaptures  - i.e. fish that are recaptured many times at a particular location rather than moving on upstream from the campground (Figure Camp.serial.recaptures) or the canyon (Figure Canyon.serial.recaptures) immediately following tagging. In the analysis we will be using only the first recapture, so this additional information is thrown out.

```{r Figure Camp.serial.recaptures, echo=F, message=F, warning=F, fig.width=6, fig.height= 5, fig.cap="Figure: Serial recaptures of fish at the Campground."}
plot.camp.recaps

```


```{r Figure Canyon.serial.recaptures, echo=F, message=F, warning=F, fig.width=6, fig.height= 5, fig.cap="Figure: Serial recaptures of fish at the Canyon."}
plot.canyon.recaps
...
We can see how well these estimates track with an external, related data source. We can start with Tyee for Coho (Figure TYEE), since the Bulkley population is a large component of the Skeena coho.

```{r Figure TYEE, echo=F, message=F, warning=F, fig.width=6, fig.height=5,fig.cap="Figure: Total of daily index for coho at Tyee by year."}
...
There is no apparent need for stratification by time, since most of the stratified estimates are similar to a fully pooled Petersen estimate. **KP: Are the estimates of error more realistic in the stratified method.**
