...
- Naniak Creel sockeye counts added in. Not as bad a Toboggan Creek, but the
...
- I inserted records into the *Tag_data_Sampling_Date* table for sample_ids 8 and 676 with imputed dates. So now 
there are no fish with missing dates.**
...
There were a few sampling times (*Sample_Id* variable that links to time/date/location of sampling) 
where no date is recorded. This occurred mostly for Sockeye Salmon.
**I inserted records into the Tag_data_Sampling_Date table for sample_ids 8 and 676 with imputed dates. So now 
there are no fish with missing dates.**
...
<!-- The values for missing dates are from two *Sample_IDs* -->
...
Virtually no Chinook Salmon were handled from 2021:2016 and the number of steelhead handed at the *Campground* in 2019 to 2021
is very small.
...
I will ignore the second tag when constructing capture-histories, so there may be a few fish which lost tags
...
tag.num.sum <- as.data.frame(xtabs(~AppliedTagNumberPresent+RecapturedTagNumberPresent, 
                     data=all.fish, exclude=NULL, na.action=na.pass))
tag.num.sum
```
...
Many of these records appear double tagged fish where the second tag is listed in the
*RecapturedTagNumber* field (*TagStatus* set to *A2*).
The remained appear to be recaptures of fish
where the old tags was replaced by a new tag (*TagStatus* set to *AR*); and 
some may be recaptures of double tagged fish (*TagStatus* set to *R*) or fish tagged in previous years (for steelhead).

**The database needed to be corrected to deal with these cases in a more transparent fashion**
...
where more than one color is associated with a tag number. 
...
Unless the tag number is recorded in the *RecapturedTagNumber* field, these records will
be ignored.
...
**Does this match the DFO statistical week?**
...
**Need to convert to DFO statistical week**
...
date.sum <- plyr::ddply(all.fish, c("Species","Year"), function(x){
  ...
  if(x$Species[1] %in% c("CO","SK","ST")){
     ...
     select <- x$TagStatus %in% c(NA, "R","A","AR")  & x$Location_Code=="Canyon"
...
We will use the captures and recaptures of fish to create a capture history for each fish which is the 
first step in the capture-recapture analysis.

## Use tag number only to match across capture-locations and dates

Because of problems with the color of tag not always being recorded or different colors appears to be used
for the same tag within a year, the capture history will be based on the tag number only (i.e., color will be ignored).
...
We will construct the two digit capture history representing tagging on the *Campground* and capture
in the *Canyon* using ONLY the tag number within species-year combination.
This will deal with the majority of cases where the tag number is recorded but not the color; 

For Species CO, SK, ST.

- If a tag number is present, and if the *TagStatus* is either *A* or *AR*, and the location
of the fish handing is *Campground*, then 
this is treated as a capture at occasion 1 (first digit of history set to 1)
- If a tag number is present and if the *TagStatus* is either *A* or *R*, and the location of the 
of the fish handling is *Canyon*, 
...
cap.hist <- plyr::ddply(all.fish, c("Species","Year","myTagNumber"), function(x){
   ...
   if(x$Species[1] %in% c("CO","SK","ST") & is.na(x$myTagNumber[1])){  # batch marks or first time captures in Canyon
     ...
     select <- x$TagStatus %in% c(NA,"A","AR") & x$Location_Code=="Canyon"
     x <- x[select,]
     freq <- 1
     hist <- rep("01", nrow(x))
...
all.fish$SYT <- paste(all.fish$Species, all.fish$Year, all.fish$myTagNumber, sep=".")
select <- all.fish$Species %in% c("CO","SK","ST") & all.fish$SYT %in% temp$SYT  |
          all.fish$Species %in% c("CH")           & 
               !(all.fish$TagStatus %in% c("A","AR") & all.fish$Location_Code=="Campground"  |
                all.fish$TagStatus %in% c("R")      & all.fish$Location_Code=="Canyon"      |
                all.fish$TagStatus %in% c("A", NA)  & all.fish$Location_Code=="Canyon") 
...
Notice that something has gone wrong with
the estimates of abundance for *CH* because most of the statistics (e.g., $n_1$)
...
A $k \time 2$ contingency table is created, where $k$ is the number of recapture strata:
...
Then the usual chi-square test for equal proportions across strata can be applied. 
In cases where the numbers in the contingency table are small, the Fisher Exact Test
for equal proportions can be applied

Here is the summary of the results of a chi-square and Fisher exact test for equal marked fractions.
The complete set of contingency tables and test statistics is presented in Appendix B.
...
A $k \time 2$ contingency table is created, where $k$ is the number of release strata,
...
The Toboggan Creek (*T.Creek*) data is split into two files
...
xtabs(total.coho~Year, data=t.markedunmarked, exclude=NULL, na.action=na.pass)
...
There were several records where a tagged fish was recaptured, but the tag number could not
be read. 

```{r echo=FALSE, message=FALSE, warning=FALSE}
select <- is.na(t.tagrecoveries$RecapturedTagNumber)
sum(select)
t.tagrecoveries[select,]
...
*Campground* or *Canyon*.
There were are number of tag numbers that could not be matched.

```{r echo=FALSE, message=FALSE, warning=FALSE}
t.tagrecoveries$SYT <- paste0("CO",".",t.tagrecoveries$Year,".",t.tagrecoveries$RecapturedTagNumber)

applied <- apply(outer(t.tagrecoveries$SYT, all.fish$SYT, "=="),1,any, na.rm=TRUE)
sum(!applied)
...
all.fish[select, c("Year","Species","TagStatus","AppliedColor","AppliedTagNumber","RecapturedTagNumber","Location_Code")]
...
There were also some fish whose tag number was not in the tagging data base.

```{r echo=FALSE, message=FALSE, warning=FALSE}
select2 <- !t.notapplied$RecapturedTagNumber %in% all.fish[select, "myTagNumber"]

t.notapplied[select2,]



```

This was due to a number of different reasons, some of which will require a fix to the database

- Tag 45616. The record for tag number 45617 has a comment saying that 45617 was a double
tag for 45616, but there is no record for tag number 45616. **This needs to be fixed in the database**.
- No records anywhere for Tag numbers  45745, 45750, 45752, 545581, 523410, 542231.

We will again treat such recaptures as unmarked fish.
...
all.fish.red <- all.fish[ all.fish$Species == "CO" &
                          all.fish$Year    >= min(t.markedunmarked$Year) &
                          all.fish$Year    <= max(t.markedunmarked$Year),]
...
Here are the tag histories from the first analysis where
recaptures only at the *Canyon* were considered

```{r echo=FALSE, message=FALSE, warning=FALSE}
xtabs(freq~ Year+hist, data=cap.hist[cap.hist$Species=="CO" &
                                    cap.hist$Year >= min(t.cap.hist$Year) &
                                    cap.hist$Year <= max(t.cap.hist$Year),])
```
...
and 23 recaptures were found in 1053 examined fish. This gives a Petersen estimator of
$$\widehat{N}_2 = \frac{1053 \times 317}{23}=14513$$ fish.
...
- Unclear why the T.Creek analysis fails so badly. Perhaps the tagging operations the Canyon miss most
